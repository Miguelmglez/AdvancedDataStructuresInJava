...
public abstract class HashTableMap<K, V> implements Map<K, V> {
    ...
    private class HashTableMapIterator<T, U> implements Iterator<Entry<T, U>> {
        ...
        public Entry<T, U> next() {
            if (hasNext()) {
                ...
                throw new InvalidEntryException ("The map has not more elements");
            }
        }
     ...
     * @return
     */
    private HashEntryIndex findEntry(K key) throws InvalidKeyException { //Modificado a public para uso en LP,...
        int avail = -1;
        checkKey(key);
        int i = hashValue(key);
        final int j = i;
        ...
        return new HashEntryIndex(avail, false); // first empty or available
        // slot
    }
