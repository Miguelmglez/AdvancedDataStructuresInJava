...
public class LinkedBinaryTree<E> implements BinaryTree<E> {
	...
	public void subTree(Position<E> v) {
        ...
        public Position<E> removeSubtree(Position<E> p) throws InvalidPositionException{ //Modificado, bien
            BTNode<E> node = checkPosition(p);
            BTNode<E> padre = node.getParent();  //Puntero al padre
            size -= calculateSize(node);  //Actualizo tamaño del arbol
            if(padre == null){      //Si el elemento pasado es raiz
                this.root = null;           //Pongo la raiz a null
                this.size = 0;              //Actualizo tamaño del arbol a 0
            }
            else{
                if(padre.getLeft() == node){        //Si el nodo es hijo izquierdo
                    padre.setLeft(null);    //Quito la referencia al hijo y asi elimino el subarbol
                    node.setParent(null);   //Quito la referencia del nodo a su padre
                }
                else{   //Caso contrario, eres hijo derecho
                    padre.setRight(null);       //Quito la referencia al hijo y asi elimino el subarbol
                    node.setParent(null);       //Quito la referencia del nodo a su padre
                }
                
            }
            
            return node;   //Devolvemos el subarbol eliminado
        }
}