...
public class LinkedBinarySearchTree<E> implements BinarySearchTree<E> {
	...
	private class BSTIterator<T> implements Iterator<Position<T>> {
		...
		public Position<T> next() {
			if(nonVisited == 0){
				throw new InvalidPositionException("The BST has not more elements");
			}
    ...
    public Iterable<Position<E>> findRange (E minValue, E maxValue) {
            ...
            addAux(l, this.binTree.root(), minValue, maxValue);
            return l;
    }  
         
         protected void addAux(List<Position<E>> l, Position<E> pos, E min, E max) {  //pos es la raiz
        if (this.binTree.isLeaf(pos))
            return;
       
        if (!this.binTree.isLeaf(pos)) { // Encontramos entry con key igual a k
            //Comparator devuelve 0 si son iguales, +1 si es mayor; -1 si es menor
            if ((this.comparator.compare(min,pos.getElement())<=0)  && (this.comparator.compare(pos.getElement(),max)<=0)) {
                //Si valor minimo es igual que elemento raiz (pos) o es menor que el elemento raiz
                //y ademas el elemento raiz es menor o igual que el maximo, lo AÑADO
                l.add(pos);
                addAux(l, this.binTree.left(pos), min, max);
                addAux(l, this.binTree.right(pos), min, max);
            }else {
